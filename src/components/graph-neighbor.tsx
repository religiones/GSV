...
import { scaleLinear, min, max, dispatch } from 'd3';
...
const GraphNeighbor: React.FC<{}> = () => {
    ...
    const [neighborCommunity, setNeighborCommunity] = useState<Community[]>([]);
    ...
    useEffect(()=>{
        if(graph != null && neighborCommunity.length != 0){
            graph.on("node:dblclick",(e: IG6GraphEvent)=>{
                const graph = e.item?.getModel();
                neighborCommunity.forEach((community:Community)=>{
                    if(community.id === Number(graph?.id)){
                        selectNeighbor.push(community);
                    }
                });
    ...
    const initNeighborGraph = (id:number, neighbor: number[])=>{
